...
class BaseTrainer:
    ...
    def optimizer_step(self):
        """Perform a single optimizer step with optional RGB gradient surgery on shared params."""
        from ultralytics.utils.torch_utils import unwrap_model

        if self.scaler:
            # 1) unscale first (so surgery writes true grads, not scaled grads)
            self.scaler.unscale_(self.optimizer)

            # 2) RGB surgery hook (only if provided)
            m = unwrap_model(self.model)
            gb = getattr(m, "grad_balancer", None)
            if gb is not None and hasattr(gb, "apply_after_unscale"):
                shared = getattr(m, "grad_balance_shared_params", None)
                if shared is None:
                    raise RuntimeError("RGBBalancer需要 model.grad_balance_shared_params，但你没在 train.py 里挂上。")
                gb.apply_after_unscale(shared)

            # 3) clip
            if getattr(self.args, "clip_grad", 0):
