...
class SegmentPose(Detect):
    """YOLO segmentation + pose head in one forward."""
    ...
    def forward(self, x):
        ...
        kpt = torch.cat([self.cv5[i](x[i]).view(bs, self.nk, -1) for i in range(self.nl)], 2)
        ...
        # ✅ 推理：det + mask coeff + keypoints，保证 NMS 后 extra 里真有 nm+nk
        y = torch.cat([x[0], mc, pred_kpt], 1)
        return y, (x[1], mc, p, kpt)
