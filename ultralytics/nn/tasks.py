    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, SegmentPose, YOLOESegment, Pose, OBB)) else self.forward(x)
...
class SegmentationModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        head = self.model[-1]
        if isinstance(head, SegmentPose):
...
class SegPoseModel(DetectionModel):
    """YOLO SegPose model (mask + keypoints)."""

    def __init__(self, cfg="yolo11n-segpose.yaml", ch=3, nc=None, data_kpt_shape=(None, None), verbose=True):
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        # expose for predictor
        self.kpt_shape = list(data_kpt_shape) if any(data_kpt_shape) else getattr(self.model[-1], "kpt_shape", None)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, SegmentPose, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
...
def guess_model_task(model):
    """Guess model task from model cfg/head/filename."""
    def cfg2task(cfg):
        m = cfg["head"][-1][-2].lower()
        if "segmentpose" in m or "segpose" in m:
            return "segpose"
        if "classify" in m:
            return "classify"
        if "detect" in m:
    ...
    if isinstance(model, (str, Path)):
        s = Path(model).stem.lower()
        if "-segpose" in s or "segpose" in s:
            return "segpose"
        if "-seg" in s:
            return "segment"
        if "-cls" in s:
    ...
    LOGGER.warning("Unable to automatically guess model task, assuming 'detect'.")
    return "detect"
