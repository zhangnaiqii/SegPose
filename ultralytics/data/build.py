...
from ultralytics.data.dataset import GroundingDataset, YOLODataset, YOLOMultiModalDataset, SegPoseDataset
...
from ultralytics.utils import RANK, colorstr, YAML
...
def build_yolo_dataset(cfg, img_path, batch, data, mode="train", rect=False, stride=32, multi_modal=False):
    ...
    # SegPose: allow pose yaml path from either data.yaml or cfg overrides
    pose_yaml_path = (
        getattr(cfg, "pose_data", None)
        or data.get("pose_data", None)
        or data.get("pose", None)
        or data.get("pose_yaml", None)
    )

    # If user is doing explicit segpose task, pose_yaml_path is mandatory
    if getattr(cfg, "task", None) == "segpose" and not pose_yaml_path:
        raise KeyError("SegPoseDataset requires a pose yaml path via cfg.pose_data or data['pose_data'].")

    # Backward-compatible: still allow segment task + pose_data in data.yaml
    if (getattr(cfg, "task", None) == "segpose") or (cfg.task == "segment" and pose_yaml_path):
        pose_yaml = check_yaml(pose_yaml_path)
        pose_data = yaml_load(pose_yaml)
        return SegPoseDataset(
            img_path=img_path,
            imgsz=cfg.imgsz,
            batch_size=batch,
            augment=mode == "train" and bool(getattr(cfg, "augment", True)),
            hyp=cfg,
