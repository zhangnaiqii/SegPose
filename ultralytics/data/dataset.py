...
class SegPoseDataset(YOLODataset):
    """Dataset that merges YOLO-Seg labels + YOLO-Pose labels by image stem."""

    def __init__(self, *args, data=None, task="segpose", mode="train", **kwargs):
        # --- accept pose yaml/dict either from data or kwargs ---
        pose_data_arg = kwargs.pop("pose_data", None)   # may be dict (already loaded)
        pose_yaml_arg = kwargs.pop("pose_yaml", None)   # may be str/path

        if not isinstance(data, dict):
            raise TypeError("SegPoseDataset requires seg data to be a dict loaded from a data.yaml.")

        self.mode = mode

        # Resolve pose yaml path / pose data dict
        pose_yaml_path = None
        if isinstance(pose_yaml_arg, (str, Path)):
            pose_yaml_path = str(pose_yaml_arg)

        if isinstance(pose_data_arg, dict):
            self.pose_data = pose_data_arg
        else:
            # prefer pose_yaml_arg, else fallback to data['pose_data']
            if pose_yaml_path is None:
                pose_yaml_path = data.get("pose_data")
            if not pose_yaml_path:
                raise KeyError("SegPoseDataset requires pose yaml via data['pose_data'] or kwarg pose_yaml/pose_data.")
            pose_yaml_path = check_yaml(pose_yaml_path)
            self.pose_data = yaml_load(pose_yaml_path)

        if "kpt_shape" not in self.pose_data:
            raise KeyError("No `kpt_shape` in pose yaml/data.")

        # build pose split image path
        pose_root = Path(self.pose_data.get("path", ""))
        if pose_root and not pose_root.is_absolute():
            if not pose_yaml_path:
                raise ValueError("pose_data['path'] is relative but pose yaml path is unknown. Pass pose_yaml.")
            pose_root = (Path(pose_yaml_path).parent / pose_root).resolve()

        pose_split = self.pose_data.get(mode)
        if pose_split is None:
            raise KeyError(f"pose yaml missing split '{mode}'.")

        pose_img_path = str((pose_root / pose_split) if pose_root else (Path(pose_yaml_path).parent / pose_split))

        # load pose labels once (using plain YOLODataset with task='pose')
        pose_kwargs = dict(kwargs)
        # ✅关键：避免把 seg 数据集的参数重复塞给 pose_ds
        for k in ("img_path", "data", "task", "mode", "pose_data", "pose_yaml"):
            pose_kwargs.pop(k, None)
        pose_kwargs["augment"] = False  # only need labels
        pose_ds = YOLODataset(img_path=pose_img_path, data=self.pose_data, task="pose", **pose_kwargs)
        pose_labels = getattr(pose_ds, "labels", None) or pose_ds.get_labels()
        self.pose_map = {Path(x["im_file"]).stem: x for x in pose_labels}

        # inject pose meta into main data for downstream (kpt_shape/flip_idx)
        data = dict(data)
        data["kpt_shape"] = self.pose_data["kpt_shape"]
        if "flip_idx" in self.pose_data:
            data.setdefault("flip_idx", self.pose_data["flip_idx"])

        # init as pure segment dataset (so it can read polygon labels)
        super().__init__(*args, data=data, task="segment", **kwargs)

        # enable keypoints output in transforms
        self.use_keypoints = True

        # ✅你这版框架里 super() 之后可能没有 self.hyp，这里强行补齐
        if not hasattr(self, "hyp") or self.hyp is None:
            self.hyp = kwargs.get("hyp", None)
        if self.hyp is None:
            raise AttributeError("SegPoseDataset missing hyp. Ensure build_yolo_dataset passes hyp=cfg.")
    ...
    def get_labels(self):
        ...
        def _xywh_to_xyxy(xywh: np.ndarray) -> np.ndarray:
            xyxy = xywh.astype(np.float32, copy=True)
        ...
        kpt_shape = self.pose_data.get("kpt_shape", (0, 0))
        nkpt, ndim = int(kpt_shape[0]), int(kpt_shape[1])

        for lb in seg_labels:
            stem = Path(lb["im_file"]).stem
            plb = self.pose_map.get(stem)
            if plb is None:
                raise FileNotFoundError(f"Missing pose label for image stem={stem}")

            n_seg = len(lb["cls"])
            n_pose = len(plb["cls"])
            pose_kpts = plb.get("keypoints", None)
            if pose_kpts is None or pose_kpts.size == 0:
                pose_kpts = np.zeros((n_pose, nkpt, ndim), dtype=np.float32)

            if n_seg == 0:
                lb["keypoints"] = pose_kpts[:0]
                continue

            if n_pose == 0:
                lb["keypoints"] = np.zeros((n_seg, nkpt, ndim), dtype=pose_kpts.dtype)
                LOGGER.warning(f"{self.prefix}{stem}: pose labels empty, using masks only (no keypoint supervision).")
                continue

            # Match instances by bbox IoU to align keypoints with segmentation instances.
            seg_xyxy = _xywh_to_xyxy(lb["bboxes"])
            pose_xyxy = _xywh_to_xyxy(plb["bboxes"])
            iou = _box_iou(seg_xyxy, pose_xyxy)

            seg_cls = lb["cls"].reshape(-1)
            pose_cls = plb["cls"].reshape(-1)
            if seg_cls.size and pose_cls.size:
                iou *= (seg_cls[:, None] == pose_cls[None, :]).astype(np.float32)

            order = np.full(n_seg, -1, dtype=int)
            used_seg, used_pose = set(), set()
            flat = np.argsort(iou.ravel())[::-1]
            for idx in flat:
                i, j = divmod(int(idx), n_pose)
                if i in used_seg or j in used_pose:
                    continue
                if iou[i, j] <= 0:
                    continue
                order[i] = j
                used_seg.add(i)
                used_pose.add(j)
                if len(used_seg) == n_seg:
                    break

            kpts_out = np.zeros((n_seg, nkpt, ndim), dtype=pose_kpts.dtype)
            matched = order >= 0
            if matched.any():
                kpts_out[matched] = pose_kpts[order[matched]]
            lb["keypoints"] = kpts_out

            unmatched_seg = int((~matched).sum())
            if unmatched_seg:
                best_iou = iou.max(axis=1) if n_pose else np.zeros(n_seg)
                # LOGGER.warning(
                #     f"{self.prefix}{stem}: {unmatched_seg}/{n_seg} seg instances without keypoints, "
                #     f"best IoU per seg={best_iou}"
                # )
            if n_pose > len(used_pose):
                LOGGER.warning(
                    f"{self.prefix}{stem}: {n_pose - len(used_pose)} pose instances unused (no matching mask)."
                )
