...
def autocast_float16(enabled: bool, device: str = "cuda"):
...
def autocast(enabled: bool, device: str = "cuda"):
    ...
    Get the appropriate autocast context manager based on PyTorch version and AMP setting."""
    if TORCH_1_13:
        # PyTorch autocast 默认 CUDA dtype 是 float16，这里强制改成 bfloat16
        if enabled and device == "cuda":
            return torch.amp.autocast(device, enabled=True, dtype=torch.bfloat16)
        return torch.amp.autocast(device, enabled=enabled)
    else:
        assert AssertionError('autocast only supports PyTorch 1.13 or newer')
