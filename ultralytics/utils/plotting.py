...
def plot_results(file: str = "path/to/results.csv", dir: str = "", on_plot: Callable | None = None):
    ...
    conflict_seg_keys = ["conflict/seg_pose_cos", "conflict/seg_pose_angle", "conflict/seg_pose_neg"]
    conflict_det_keys = ["conflict/det_seg_pose_cos", "conflict/det_seg_pose_angle", "conflict/det_seg_pose_neg"]
    seg_fig = seg_ax = det_fig = det_ax = None

    def _smooth_line(y):
        if np.isfinite(y).sum() < 2:
            return y
        y_mean = np.nanmean(y)
        y_fill = np.nan_to_num(y, nan=y_mean if np.isfinite(y_mean) else 0.0)
        return gaussian_filter1d(y_fill, sigma=3)
    for i, f in enumerate(files):
        try:
            ...
            if i == 0:
                ...
                ax = ax.ravel()
                if all(k in data.columns for k in conflict_seg_keys):
                    seg_fig, seg_ax = plt.subplots(1, len(conflict_seg_keys), figsize=(10, 3), tight_layout=True)
                    seg_ax = seg_ax.ravel()
                if all(k in data.columns for k in conflict_det_keys):
                    det_fig, det_ax = plt.subplots(1, len(conflict_det_keys), figsize=(10, 3), tight_layout=True)
                    det_ax = det_ax.ravel()
            x = data.select(data.columns[0]).to_numpy().flatten()
            for i, j in enumerate(columns):
                y = data.select(j).to_numpy().flatten().astype("float")
                ax[i].plot(x, y, marker=".", label=f.stem, linewidth=2, markersize=8)  # actual results
                ax[i].plot(x, _smooth_line(y), ":", label="smooth", linewidth=2)  # smoothing line
                ax[i].set_title(j, fontsize=12)
            if seg_ax is not None:
                for k, j in enumerate(conflict_seg_keys):
                    y = data.select(j).to_numpy().flatten().astype("float")
                    seg_ax[k].plot(x, y, marker=".", label=f.stem, linewidth=2, markersize=6)
                    seg_ax[k].plot(x, _smooth_line(y), ":", label="smooth", linewidth=2)
                    seg_ax[k].set_title(j, fontsize=11)
            if det_ax is not None:
                for k, j in enumerate(conflict_det_keys):
                    y = data.select(j).to_numpy().flatten().astype("float")
                    det_ax[k].plot(x, y, marker=".", label=f.stem, linewidth=2, markersize=6)
                    det_ax[k].plot(x, _smooth_line(y), ":", label="smooth", linewidth=2)
                    det_ax[k].set_title(j, fontsize=11)
    ...
    if seg_fig is not None:
        seg_ax[-1].legend()
        seg_name = save_dir / "conflict_seg_pose.png"
        seg_fig.savefig(seg_name, dpi=200)
        plt.close(seg_fig)
        if on_plot:
            on_plot(seg_name)
    if det_fig is not None:
        det_ax[-1].legend()
        det_name = save_dir / "conflict_det_seg_pose.png"
        det_fig.savefig(det_name, dpi=200)
        plt.close(det_fig)
        if on_plot:
            on_plot(det_name)
