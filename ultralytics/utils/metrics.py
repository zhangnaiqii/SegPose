...
def get_oks_sigma(kpt_shape, kpt_sigma=None) -> np.ndarray:
    """
    Get OKS sigma with length == num_keypoints (K).

    Args:
        kpt_shape: (K, D) from data.yaml, e.g. [17, 3] or [4, 3]
        kpt_sigma: optional list/tuple/ndarray with length K from data.yaml

    Returns:
        np.ndarray: shape (K,) float32

    Rules:
        - If kpt_sigma is provided: must have length K, values > 0.
        - If not provided:
            - K==17: use COCO default OKS_SIGMA
            - else: raise ValueError (force user to define sigma in yaml)
    """
    if kpt_shape is None or len(kpt_shape) < 1:
        raise ValueError(f"Invalid kpt_shape={kpt_shape}, expected like [K, D].")
    nkpt = int(kpt_shape[0])
    if nkpt <= 0:
        raise ValueError(f"Invalid kpt_shape={kpt_shape}, K must be > 0.")

    if kpt_sigma is None:
        if int(OKS_SIGMA.shape[0]) == nkpt:
            return OKS_SIGMA.astype(np.float32).copy()
        raise ValueError(
            f"Custom keypoints detected (K={nkpt}) but data.yaml has no kpt_sigma. "
            f"Please set `kpt_sigma:` with length {nkpt} in your pose data yaml."
        )

    if not isinstance(kpt_sigma, (list, tuple, np.ndarray)):
        raise TypeError(f"kpt_sigma must be list/tuple/ndarray, got {type(kpt_sigma)}")
    if len(kpt_sigma) != nkpt:
        raise ValueError(f"kpt_sigma length mismatch: len(kpt_sigma)={len(kpt_sigma)} vs K={nkpt}")

    sigma = np.array(kpt_sigma, dtype=np.float32)
    if not np.all(np.isfinite(sigma)):
        raise ValueError(f"kpt_sigma contains non-finite values: {sigma}")
    if not np.all(sigma > 0):
        raise ValueError(f"kpt_sigma must be all > 0, got: {sigma}")
    return sigma
...
def kpt_iou(
    ...
    Args:
        kpt1 (torch.Tensor): Ground truth keypoints, shape (N, K, D) where D is 2 or 3.
        kpt2 (torch.Tensor): Predicted keypoints, shape (M, K, D) where D is 2 or 3.
        area (torch.Tensor): Ground truth areas, shape (N,).
        sigma (list[float]): Keypoint sigmas, length K.
        eps (float): Small value to avoid division by zero.

    Returns:
        torch.Tensor: OKS similarities, shape (N, M).
    """
    if kpt1.ndim != 3 or kpt2.ndim != 3:
        raise ValueError(f"kpt_iou expects kpt1/kpt2 with shape (N,K,D). Got {kpt1.shape}, {kpt2.shape}")
    if kpt1.shape[1] != kpt2.shape[1]:
        raise ValueError(f"K mismatch: kpt1 K={kpt1.shape[1]} vs kpt2 K={kpt2.shape[1]}")
    if kpt1.shape[2] not in (2, 3) or kpt2.shape[2] not in (2, 3):
        raise ValueError(f"Only D=2 or D=3 supported. Got D={kpt1.shape[2]} and D={kpt2.shape[2]}")

    K = int(kpt1.shape[1])
    if not isinstance(sigma, (list, tuple, np.ndarray)):
        raise TypeError(f"sigma must be list/tuple/ndarray, got {type(sigma)}")
    if len(sigma) != K:
        raise ValueError(f"sigma length mismatch: len(sigma)={len(sigma)} vs K={K}")

    sigma_t = torch.as_tensor(sigma, device=kpt1.device, dtype=kpt1.dtype)  # (K,)
    if torch.any(~torch.isfinite(sigma_t)) or torch.any(sigma_t <= 0):
        raise ValueError(f"sigma must be finite and >0, got {sigma_t}")

    # distance squared: (N, M, K)
    d = (kpt1[:, None, :, 0] - kpt2[..., 0]).pow(2) + (kpt1[:, None, :, 1] - kpt2[..., 1]).pow(2)

    # visibility mask from gt: (N, K)
    if kpt1.shape[2] == 3:
        kpt_mask = kpt1[..., 2] != 0
    else:
        kpt_mask = torch.ones((kpt1.shape[0], K), device=kpt1.device, dtype=torch.bool)

    # oks exponent term
    e = d / ((2 * sigma_t).pow(2) * (area[:, None, None] + eps) * 2)
...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    def process(self, save_dir: Path = Path("."), plot: bool = False, on_plot=None) -> dict[str, np.ndarray]:
        """Process the stored statistics and update the box metrics."""

        # Figure out tp width (IoU thresholds count). Default is 10 (COCO: 0.50:0.95 step 0.05).
        tp_width = 10
        for k, v in self.stats.items():
            if k.startswith("tp") and len(v):
                a = v[0]
                if hasattr(a, "ndim") and a.ndim == 2:
                    tp_width = a.shape[1]
                break

        # Safe concatenate: empty lists become empty arrays with correct shapes/dtypes
        ...
        for k, v in self.stats.items():
            ...
            if k.startswith("tp"):
                stats[k] = np.zeros((0, tp_width), dtype=bool)
            elif k in {"pred_cls", "target_cls", "target_img"}:
                stats[k] = np.zeros((0,), dtype=np.int64)
            elif k == "conf":
                stats[k] = np.zeros((0,), dtype=np.float32)
            else:
                stats[k] = np.zeros((0,), dtype=np.float32)
...
class SegPoseMetrics(SegmentMetrics):
    """Box + Mask + Pose metrics."""

    def __init__(self, names: dict[int, str] = {}) -> None:
        super().__init__(names)
        self.pose = Metric()
        self.task = "segpose"
        self.stats["tp_p"] = []  # pose TP stats
        self.stats["target_cls_p"] = []  # pose GT classes (filter instances without keypoints)

    def update_stats(self, stat: dict[str, Any]) -> None:
        # 兼容 SegmentationValidator 的 stat（它不会提供 tp_p）
        for k in ("tp", "conf", "pred_cls", "target_cls", "target_img", "tp_m"):
            self.stats[k].append(stat[k])
        if "tp_p" in stat:
            self.stats["tp_p"].append(stat["tp_p"])

    def process(self, save_dir: Path = Path("."), plot: bool = False, on_plot=None) -> dict[str, np.ndarray]:
        stats = super().process(save_dir, plot, on_plot=on_plot)  # box + mask

        # If validator didn't provide pose TP stats, tp_p may be empty/misaligned.
        if "tp_p" not in stats:
            return stats

        n = int(stats["conf"].shape[0]) if "conf" in stats else 0
        tpw = 10
        if "tp" in stats and hasattr(stats["tp"], "ndim") and stats["tp"].ndim == 2:
            tpw = int(stats["tp"].shape[1])
        elif hasattr(stats["tp_p"], "ndim") and stats["tp_p"].ndim == 2 and stats["tp_p"].shape[1] > 0:
            tpw = int(stats["tp_p"].shape[1])

        # Ensure tp_p is (n, tpw), so ap_per_class(tp_p, conf, ...) indexing never breaks.
        if (not hasattr(stats["tp_p"], "ndim")) or stats["tp_p"].ndim != 2 or int(stats["tp_p"].shape[0]) != n:
            stats["tp_p"] = np.zeros((n, tpw), dtype=bool)

        target_cls = stats.get("target_cls_p", stats["target_cls"])
        results_pose = ap_per_class(
            stats["tp_p"],
            stats["conf"],
            stats["pred_cls"],
    ...
    def mean_results(self) -> list[float]:
        return SegmentMetrics.mean_results(self) + self.pose.mean_results()

    def class_result(self, i: int) -> list[float]:
        return SegmentMetrics.class_result(self, i) + self.pose.class_result(i)

    @property
    def maps(self) -> np.ndarray:
        return SegmentMetrics.maps.fget(self) + self.pose.maps

    @property
    def fitness(self) -> float:
        return self.pose.fitness() + SegmentMetrics.fitness.fget(self)
